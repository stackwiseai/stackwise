...
const transformToChatMessageHistory = (chatString: string): ChatMessageHistory => {
...
export async function POST(req: Request) {
  ...
  if (!file || !(file instanceof Blob)) {
    return new Response(JSON.stringify({ error: 'No PDF file provided or file is not a Blob' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
  }

  if (!question || typeof question !== 'string') {
    return new Response(JSON.stringify({ error: 'Question not provided or not a string' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
  }
  ...
  try {
    ...
    const memoryVectorStore = await MemoryVectorStore.fromDocuments(allDocs, new OpenAIEmbeddings({ modelName: "text-embedding-ada-002" }));
    const llm = new ChatOpenAI({ });
    const chatHistory = chatHistoryValue ? transformToChatMessageHistory(chatHistoryValue) : new ChatMessageHistory();
    ...
    console.error(error);
    return new Response(JSON.stringify({ error: 'Error processing PDF: ' + error.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}